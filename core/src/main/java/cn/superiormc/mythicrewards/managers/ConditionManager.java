...
import cn.superiormc.mythicrewards.objects.conditions.*;
...
public class ConditionManager {
    ...
    public boolean checkBoolean(ObjectSingleCondition condition, Player player, TrackerResult result) {
        for (AbstractCheckCondition checkCondition : conditions.values()) {
            String type = condition.getString("type");
            if (checkCondition.getType().equals(type) && !checkCondition.checkCondition(condition, player, result)) {
                return false;
            }
        }
        return true;
