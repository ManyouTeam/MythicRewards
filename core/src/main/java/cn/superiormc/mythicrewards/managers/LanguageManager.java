...
public class LanguageManager {
    ...
    private void initLanguage() {
        this.file = new File(MythicRewards.instance.getDataFolder() + "/languages/" + ConfigManager.configManager.getString("config-files.language", "en_US") + ".yml");
        if (!file.exists()){
            this.file = new File(MythicRewards.instance.getDataFolder(), "message.yml");
            if (!file.exists()) {
                MythicRewards.methodUtil.sendMessage(null, TextUtil.pluginPrefix() + " §cWe can not found your message file, " +
        ...
        InputStream is = MythicRewards.instance.getResource("languages/en_US.yml");
        if (is == null) {
            return;
        }
        this.tempFile = new File(MythicRewards.instance.getDataFolder(), "tempMessage.yml");
        try {
    ...
    public void sendStringText(String... args) {
        ...
        if (text == null) {
            if (this.tempMessageFile.getString(args[0]) == null) {
                MythicRewards.methodUtil.sendMessage(null, TextUtil.pluginPrefix() + " §cCan not found language key: " + args[0] + "!");
                return;
            } else {
                MythicRewards.methodUtil.sendMessage(null, TextUtil.pluginPrefix() + " §cUpdated your language file, added " +
        ...
        if (!text.isEmpty()) {
            MythicRewards.methodUtil.sendMessage(null, text);
        }
    }
    ...
    public void sendStringText(Player player, String... args) {
        ...
        if (!text.isEmpty()) {
            MythicRewards.methodUtil.sendMessage(player, text);
        }
    }
